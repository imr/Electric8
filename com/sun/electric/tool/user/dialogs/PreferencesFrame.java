...
public class PreferencesFrame extends EDialog
{
	...
	public static void preferencesCommand()
	{
		if (currentOne == null)
		{
			currentOne = new PreferencesFrame(Main.getCurrentJFrame());
		}
	...
	public PreferencesFrame(Frame parent)
	{
		...
		JFrame top = (JFrame) Main.getCurrentJFrame();
		if (top != null && Main.getEMenuBar() != null)
			addTreeNode(new EditKeyBindings(Main.getEMenuBar(), parent, true), generalSet);
...
//            System.out.println("Exception while loading plugin class "+className+": "+e.getMessage());
	...
	{
		//ManualViewer.showPreferenceHelp(currentSectionName.trim() + "/" + currentTabName); TODO: fix this
		closeDialog(null);
	}
