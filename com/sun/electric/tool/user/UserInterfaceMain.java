...
//	/** Property fired if ability to Redo changes */	public static final String propRedoEnabled = "RedoEnabled";
    ...
    private static List<JInternalFrame> windowList = new ArrayList<JInternalFrame>();
    ...
    public UserInterfaceMain(List<String> argsList, Mode mode, Main thisContain) {
        ...
        SwingUtilities.invokeLater(new InitializationRun(argsList, mode));
    }
    ...
    private class InitializationRun implements Runnable {
        ...
        InitializationRun(List<String> argsList, Mode mode) {
...
//    }
    ...
    {
        if (Job.isClientThread())
            JOptionPane.showMessageDialog(Main.getCurrentJFrame(false), message, title, JOptionPane.ERROR_MESSAGE);
        else {
            SwingUtilities.invokeLater( new Runnable() {
                public void run() {
